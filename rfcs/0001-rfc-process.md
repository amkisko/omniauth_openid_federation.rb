...
This RFC defines how omniauth_openid_federation design changes are proposed, written, numbered, and advanced.

## Motivation

Login, trust-chain failure, token verification, and the federation HTTP endpoint are the public contract. A pull request description is too small to review those together. The README mixes install steps with algorithm policy. Numbered RFCs are the unit for a change that would move the gem version for every relying party. RFC 0002 lists the 2.0.0 fail-closed checks and the surfaces that still need a later Standards Track RFC.
...
usr/docs traces record decisions. RFCs are the public, numbered discussion unit. A template without type and status is too thin. A process that needs Council and Board roles does not match this project.

## Prior art

kiskolabs/pray RFC 0001. Community RFC templates and type/status ladders were considered when assembling this form.
